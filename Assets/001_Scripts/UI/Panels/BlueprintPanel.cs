...
namespace AstraNope.UI.Panels
{
    ...
    public sealed class BlueprintPanel : PanelBase
    {
        ...
        private void Refresh()
        {
            ...
            int inProgress = total - unlocked;
            if (summary) summary.text = L10n.F("k_2bcddf1c75", unlocked, total, inProgress);
        }
        ...
        private void SelectBlueprint(int blueprintId)
        {
            ...
            {
                _notifications?.Show(L10n.T("k_cb859172df"), L10n.T("k_71d0fea9da"), "!",
            ...
            if (_buildingPlacement == null || !_buildingPlacement.TryBegin(blueprintId, out failure))
            {
                if (!string.IsNullOrWhiteSpace(failure))
                    _notifications?.Show(L10n.T("k_b9416e6ed0"), failure, "!", NotificationKind.Warning, 3f);
                return;
            }
