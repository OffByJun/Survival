...
namespace AstraNope.Gameplay.Player
{
    ...
    public sealed class BuildingPlacementController : MonoBehaviour, IBuildingPlacementService, IBuildSelectionReader
    {
        ...
        public bool TryBegin(int blueprintId, out string failureReason)
        {
            ...
            _validMessage = L10n.F("k_d4def1daec", definition.DisplayName);
            _invalidMessage = L10n.F("k_aa890b47ad", definition.DisplayName);
            _hasPublishedState = false;
            _beganFrame = Time.frameCount;
            UpdateCandidate();
            _notifications?.Show(L10n.T("k_b9416e6ed0"), L10n.T("k_988c89f32a"), "◇",
        ...
        private void TryConfirm()
        {
            if (!_canPlace)
            {
                _notifications?.Show(L10n.T("k_5c67ae9204"), L10n.T("k_d5b52de0d5"), "!",
            ...
            {
                _notifications?.Show(L10n.T("k_c54bd3234e"), failure, "!", NotificationKind.Warning, 3f);
                Cancel();
                return;
            }
            ...
            _notifications?.Show(L10n.T("k_f8c12847df"), _active.DisplayName, "◆", NotificationKind.Info, 3f);
            Cancel();
        }
        ...
        private bool HasRequiredMaterials(int blueprintId, out string failure)
        {
            ...
            foreach (var pair in totals)
            {
                ...
                failure = L10n.F("k_d8743acab0", pair.Key, pair.Value);
                return false;
            }
            return true;
        }
