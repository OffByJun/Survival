...
{
    [InitializeOnLoad]
    internal static class InventoryUGUIBuilder
    {
        ...
        private static void Build(InventoryPanel panel, bool saveScene)
        {
            ...
            if (craftPanel) UIInputBindingEditor.EnsureNavigationForPanels(panel, craftPanel);
            UIRoundedStyle.ApplyToRoot(panel.gameObject);
        ...
        private static GameObject BuildSlotPrefab()
        {
            ...
            var root = new GameObject("InventorySlot", typeof(RectTransform), typeof(CanvasRenderer), typeof(Image),
                typeof(CanvasGroup), typeof(ItemSlot), typeof(UIInteractableFX));
            root.layer = 5;
            ...
            serialized.ApplyModifiedPropertiesWithoutUndo();
            UIRoundedStyle.ApplyToRoot(root);
